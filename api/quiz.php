...
use Ucebnice\API\Quiz;

const QUIZ_DIR = __DIR__ . '/../quizzes';

$quiz = new Quiz(QUIZ_DIR);

switch ($quiz->getMethod()) {
    case "OPTIONS":
        $quiz->respondToPreflight();
        break;

    case "GET":
        $quiz->setQuizID($_GET['id'] ?? '');
        $quiz->serve();
        break;
    ...
    case "POST":
        ...
        $quiz->setQuizID($body['id'] ?? '');
        $quiz->grade($body['answers'] ?? []);
        break;

    default:
        $quiz->throwBadRequest();
        break;
}
